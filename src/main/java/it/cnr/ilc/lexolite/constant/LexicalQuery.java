...
public class LexicalQuery {
    ...
    public static final String ENTRY_VERIFIED = "SELECT ?v WHERE {"
...
//            + " PropertyValue(?l, lime:language, ?lang) }";
    ...
    // DA RIVEDERE !!!!!!!!!!!!!!!!!!!!!!!
    public static final String ADVANCED_FILTER_LEMMA = "SELECT ?le ?individual ?writtenRep ?sense ?verified ?type ?sn ?pos ?a WHERE {"
            + " PropertyValue(?le, ontolex:canonicalForm, ?individual), "
            + " PropertyValue(?le, ditmaolemon:verified, ?verified), "
            + " PropertyValue(?individual, ontolex:writtenRep, ?writtenRep), "
            + " DirectType(?le, ?type), "
